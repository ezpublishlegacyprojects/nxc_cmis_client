...
$id = isset( $userParameters['id'] ) ? urldecode( $userParameters['id'] ) : false;

if ( !$id )
{
    header( 'HTTP/1.0 500 Internal Server Error' );
    echo ezi18n( 'design/standard/ezoe', 'Invalid or missing parameter: %parameter', null, array( '%parameter' => 'id' ) );
    eZExecution::cleanExit();
}

try
{
    $object = eZAlfresco::getContentObject( $id );

    if ( !$object )
    {
        header( 'HTTP/1.0 500 Internal Server Error' );
        echo ezi18n( 'alfresco', 'Could not fetch content object by alfresco object id: %key', null, array( '%key' => $id ) );
        eZExecution::cleanExit();
    }
