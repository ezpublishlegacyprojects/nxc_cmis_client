...
if ( $http->hasPostVariable( 'ConfirmButton' ) )
{
    ...
    elseif ( strtolower( $classID ) == 'file' )
    {
        ...
        if ( !$selfUri and !$canFetch )
        {
            ...
            eZDebug::writeError( $error, 'cmis/create' );
        }
...
if ( !$objectKey )
{
    ...
    {
        eZDebug::writeError( "Class ID ($classID) is not supported", 'cmis/edit' );
...
else // If an object should be edited
{
    try
    {
        ...
        if ( !$object->hasObject() )
        {
            eZDebug::writeError( "Could not fecth object by url: $objectKey", 'cmis/edit' );
...
if ( !$classID )
{
    eZDebug::writeError( 'Class id is not defined', 'cmis/edit' );
...
$Result = array();
$Result['content'] = $tpl->fetch( 'design:cmis/edit/' . strtolower( $classID ) . '.tpl' );
