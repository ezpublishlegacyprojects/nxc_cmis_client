...
<div class="upload-view">
    {def $alfresco_url = concat( 'alfresco/upload/', $object_id, '/', $object_version, '/auto/1' )|ezurl( no )
     	 $ezoe_url = concat('ezoe/upload/', $object_id, '/', $object_version, '/auto/1' )|ezurl( no )}

    <script type="text/javascript">
    {literal}
    changeAction = function( ezoe, alfresco )
    {
        var form=document.getElementById( "EmbedForm" );
        var select=document.getElementById( "location" );
        var option = select.options[select.selectedIndex];

        if ( option.id == 'alfresco' )
        {
            form.setAttribute( 'action', alfresco );
            return;
        }
            ...
            <table class="properties">
                ...
                <tr id="embedlistsrcrow">
                    ...
                    <td colspan="2" id="embedlistsrccontainer">
                      <select onclick="changeAction( '{$ezoe_url}', '{$alfresco_url}' );" name="location" id="location" title="{'Lets you specify where in eZ Publish to store the uploaded object.'|i18n('design/standard/ezoe/wai')}">
                        ...
                        {def $alfresco_object = fetch( 'alfresco', 'object', hash( 'id', ezini( 'LocationSettings', 'RootNode', 'alfresco.ini' ) ) )}


                        {if $alfresco_object}
                        	{if ezhttp('knowledgeTreeDMS', 'session')}
                            	<option id="alfresco" value="alfresco_{$alfresco_object.childrenUri}">KnowledgeTree::{$alfresco_object.title}</option>
                            {else}
                            	<option id="alfresco" value="alfresco_{$alfresco_object.childrenUri}">Alfresco::{$alfresco_object.title}</option>
                            {/if}
                        {/if}
                      </select>
                    </td>
